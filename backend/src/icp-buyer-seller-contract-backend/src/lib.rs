...
#[ic_cdk::update]
async fn get_balance() -> Result<String, ApiError> {
    WalletServiceImpl::new(true).get_balance().await
}
...
#[ic_cdk::update]
async fn get_balance_usdc() -> Result<String, ApiError> {
    WalletServiceImpl::new(true).get_balance_usdc().await
}

#[ic_cdk::update]
async fn issue_payment(contract_id: String, address: String, amount: u64) -> Result<(), ApiError> {
    let caller = ic_cdk::caller();
    ...
    ContractServiceImpl::default()
    .with_wallet(WalletServiceImpl::new(true))
    .issue_payment(contract_id, address, amount).await
}
