...
pub trait WalletService {
    async fn get_balance(&self) -> Result<String, ApiError>;
    async fn get_address(&self) -> Result<String, ApiError>;
    async fn get_balance_usdc(&self) -> Result<String, ApiError>;
...
static TRNSK_CONTRACT_ADDRESS: &'static str = "0c86A754A29714C4Fe9C6F1359fa7099eD174c0b";
static CONTRACT_ADDRESS: &'static str = TRNSK_CONTRACT_ADDRESS;
...
fn get_rpc_service_sepolia() -> RpcService {
    ...
    RpcService::Custom(RpcApi {
        url: "https://catts-evm-proxy-2.m4xsimo.workers.dev/eth-sepolia".to_string(),
        headers: None,
    })
}
...
fn get_rpc_service_base() -> RpcService {
    ...
    RpcService::Custom(RpcApi {
        url: "https://catts-evm-proxy-2.m4xsimo.workers.dev/eth-mainnet".to_string(),
        headers: None,
    })
}
...
impl WalletService for WalletServiceImpl {
    ...
    async fn get_balance(&self) -> Result<String, ApiError> {
        let address = create_icp_signer().await.address();
