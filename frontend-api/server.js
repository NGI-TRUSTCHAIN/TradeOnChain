...
// RPC (Remote Procedure Call) - Proxy (NEED IPv6)
/*const ALCHEMY_URL = 'https://eth-sepolia.g.alchemy.com/v2/CHuiW-HPyEVyhelHUsqqZq3WHSCTZcEF';
//const ALCHEMY_URL = 'https://eth-mainnet.alchemyapi.io/v2/CHuiW-HPyEVyhelHUsqqZq3WHSCTZcEF';
const CACHE_TTL = 30; // Cache per 30 secondi
...
app.post('/rpc', async (req, res) => {
	...
	// Get cacheRequest
	const cacheRequest = await getCacheRequest(hashRequest(req.body));
	if(typeof cacheRequest!=='undefined' && cacheRequest!==null) {
		logger.debug("Cache request found valid into db...");
		const responseData = cacheRequest.response;
		logger.debug("Cached request returned the answer: "+responseData);
		res.setHeader("Content-Type", "text/plain;charset=UTF-8");
        return res.send(responseData);
    }
	// If there is already a pendingRequest, wait for its result
    if(pendingRequests.has(requestHash)) {
        logger.debug("Waiting for existing request...");
        try {
            const responseData = await pendingRequests.get(requestHash);
			logger.debug("Existing request returned the answer: "+JSON.stringify(responseData));
            res.setHeader("Content-Type", "text/plain;charset=UTF-8");
            return res.send(responseData);
        } catch (error) {
			logger.debug("Errore 1 nella richiesta RPC");
            return res.status(500).json({ error: 'Errore 1 nella richiesta RPC' });
        }
    }
	logger.debug("Cache request found not valid into db, making a new request...");
	const requestPromise = axios.post(ALCHEMY_URL, req.body).then(response => {
		const responseData = response.data;
		storeCacheRequest(requestHash, requestData, JSON.stringify(responseData));
		logger.debug("Promise return the answer: "+JSON.stringify(responseData));
		return responseData;
	}).catch((error) => {
		return Promise.reject(error);
	}).finally(() => {
		logger.debug("Deleting promise");
		pendingRequests.delete(requestHash);
	});
    pendingRequests.set(requestHash, requestPromise);
    try {
        const responseData = await requestPromise;
		logger.debug("I'm here returning the answer: "+JSON.stringify(responseData));
        res.setHeader("Content-Type", "text/plain;charset=UTF-8");
        return res.send(responseData);
    } catch (error) {

		logger.debug("Errore 2 nella richiesta RPC");
		logger.error("Errore nella richiesta RPC:", {
			message: error.message,
			stack: error.stack, // Stack trace completo
		});
        res.status(500).json({ error: 'Errore 2 nella richiesta RPC' });
    }
});
async function getCacheRequest(requestHash) {
	logger.debug('Getting cacheRequest...');
	return new Promise((resolve, reject) => {
		// Get db connection
		pool.getConnection((err, connection) => {
			if(err) {
				...
				const time = Math.floor(Date.now()/1000);
				const maxTime = time - CACHE_TTL;
				// Prepare select query
				const selectQuery = `SELECT * FROM cacheRequest WHERE requestHash = ? AND created > ?`;
				console.log(time);
				console.log(CACHE_TTL);
				console.log(maxTime);
				connection.query(selectQuery, [requestHash, maxTime], (err, rows) => {
					connection.release();
					if(err) {
						logger.error("Error getting cacheRequest:" + err);
						reject(null);
					} else {
						console.log(rows);
						if(rows.length>0) {
							const cacheRequest = rows[0];
							console.log(cacheRequest);
							resolve(cacheRequest);
						}
						resolve(null);
					}
				});
...
async function storeCacheRequest(requestHash, requestData, responseData) {
	logger.debug('Storing cacheRequest...');
	// Get db connection
	pool.getConnection((err, connection) => {
		if(err) {
			logger.error('Database connection error. Error: '+JSON.stringify(err));
			...
			// Prepare insert query
			var query = `
				INSERT INTO cacheRequest (
					\`requestHash\`, \`request\`, \`response\`, \`created\`
				) 
				VALUES (?, ?, ?, ?)
				ON DUPLICATE KEY UPDATE
			...
			// Prepare query params
			const time = Math.floor(Date.now()/1000);
			const params = [requestHash, requestData, responseData, time];
			// Perform insert query
			connection.query(query, params, (err, result) => {
				if(err) {
...
/* Transak other API
async function updateTransakWebhook() {
	logger.debug('Update Transak webhook...');
	const accessToken = await getTransakAccessToken();
	console.log(accessToken);
	try {
        const response = await axios.post(TRANSAK_API_URL+"/partners/api/v2/update-webhook-url", {
			'webhookURL': "https://testapi.tradeonchain.com/webhook/v1/transak",
		},{
            headers: {
				'Content-Type': 'application/json',
				'Accept': 'application/json',
        ...
        if(response.data && response.data) {
			logger.info("Transak webhook updated");
			console.log(response.data);
            return response.data;
        } else {
            logger.error('Error updating Transak webhook');
        }
    } catch (error) {
        logger.error('Error fetching access token:'+error);
    }
}
async function getTransakWebhooks() {
	logger.debug('Getting Transak webhooks...');
	const accessToken = await getTransakAccessToken();
	console.log(accessToken);
	try {
        const response = await axios.get(TRANSAK_API_URL+"/partners/api/v2/webhooks", {
            headers: {
				'Content-Type': 'application/json',
				'Accept': 'application/json',
                'access-token': accessToken
        ...
        if(response.data && response.data) {
            ...
            logger.error('Error getting Transak webhooks');
			console.log(response);
        }
    } catch (error) {
        logger.error('Error getting Transak webhooks:'+error);
		console.log(error);
    }
}
async function testTransakWebhook() {
	logger.debug('Testing Transak webhook...');
	const accessToken = await getTransakAccessToken();
	try {
		const response = await axios.post(TRANSAK_API_URL+"/partners/api/v2/test-webhook", {},{
            headers: {
				'Content-Type': 'application/json',
				'Accept': 'application/json',
                'access-token': accessToken
        ...
        if(response.data && response.data) {
            ...
            logger.error('Error testing Transak webhook');
        }
    } catch (error) {
		console.log(error);
        logger.error('Error testing Transak webhooks:'+error);
    }
}*/
...
async function generateContractPDF(data) {
    ...
    // Titolo principale
    page.drawText('MODELLO DI CONTRATTO INTERNAZIONALE DI VENDITA', {
        x: 50,
        y: height - 50,
        size: 18,
        font: timesBoldFont,
    ...
    // Intestazione: Informazioni sul venditore
    page.drawText('Venditore:', { x: 50, y: height - 100, size: 14, font: timesBoldFont });
    page.drawText(`${data.sellerName}`, { x: 150, y: height - 100, size: fontSize, font: timesRomanFont });
    page.drawText(`Indirizzo: ${data.sellerAddress}`, { x: 150, y: height - 120, size: fontSize, font: timesRomanFont });
    page.drawText(`Codice Fiscale: ${data.sellerTaxCode}`, { x: 150, y: height - 140, size: fontSize, font: timesRomanFont });

    // Intestazione: Informazioni sul compratore
    page.drawText('Compratore:', { x: 50, y: height - 180, size: 14, font: timesBoldFont });
    page.drawText(`${data.buyerName}`, { x: 150, y: height - 180, size: fontSize, font: timesRomanFont });
    page.drawText(`Indirizzo: ${data.buyerAddress}`, { x: 150, y: height - 200, size: fontSize, font: timesRomanFont });
    page.drawText(`Codice Fiscale: ${data.buyerTaxCode}`, { x: 150, y: height - 220, size: fontSize, font: timesRomanFont });

    // Condizioni specifiche della vendita
    page.drawText('Condizioni Specifiche della Vendita:', { x: 50, y: height - 260, size: 14, font: timesBoldFont });
    page.drawText(`Descrizione delle merci: ${data.goodsDescription}`, { x: 50, y: height - 280, size: fontSize, font: timesRomanFont });
    page.drawText(`Quantità: ${data.quantity}`, { x: 50, y: height - 300, size: fontSize, font: timesRomanFont });
    page.drawText(`Prezzo: ${data.contractPrice} ${data.currency}`, { x: 50, y: height - 320, size: fontSize, font: timesRomanFont });

    // Condizioni di pagamento
    page.drawText('Condizioni di Pagamento:', { x: 50, y: height - 360, size: 14, font: timesBoldFont });
    page.drawText(`Metodo di pagamento: ${data.paymentMethod}`, { x: 50, y: height - 380, size: fontSize, font: timesRomanFont });
    page.drawText(`Termini: ${data.paymentTerms}`, { x: 50, y: height - 400, size: fontSize, font: timesRomanFont });

    // Condizioni di consegna
    page.drawText('Condizioni di Consegna:', { x: 50, y: height - 440, size: 14, font: timesBoldFont });
    page.drawText(`Termine di consegna (Incoterms): ${data.deliveryTerms}`, { x: 50, y: height - 460, size: fontSize, font: timesRomanFont });
    page.drawText(`Data di consegna: ${data.deliveryDate}`, { x: 50, y: height - 480, size: fontSize, font: timesRomanFont });

    // Firma
    page.drawText('Luogo e Data:', { x: 50, y: height - 520, size: 14, font: timesBoldFont });
    page.drawText(`Luogo: ${data.contractPlace}`, { x: 50, y: height - 540, size: fontSize, font: timesRomanFont });
    page.drawText(`Data: ${data.contractDate}`, { x: 50, y: height - 560, size: fontSize, font: timesRomanFont });

    page.drawText('Firma del Venditore:', { x: 50, y: height - 600, size: 14, font: timesBoldFont });
    page.drawText('_______________________', { x: 200, y: height - 600, size: fontSize, font: timesRomanFont });

    page.drawText('Firma del Compratore:', { x: 50, y: height - 640, size: 14, font: timesBoldFont });
    page.drawText('_______________________', { x: 200, y: height - 640, size: fontSize, font: timesRomanFont });

    // Serializza il PDF in bytes
    const pdfBytes = await pdfDoc.save();
